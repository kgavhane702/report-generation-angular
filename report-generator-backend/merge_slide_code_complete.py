...
and completed outputs are validated so unsafe structural corruption cannot be
published. Known repairable recovery artifacts and stale section memberships
are normalized during the merge.
"""
...
INSTANCE_FOLDERS = ('slides', 'notesSlides', 'drawings', 'charts', 'comments')
IGNORABLE_RECOVERY_DIRECTORIES = frozenset(('trash', '[trash]'))

def is_ignorable_recovery_part(part_name):
    """Return True for top-level PowerPoint Trash folder variants."""
    normalized = str(part_name).replace('\\', '/').lstrip('/')
    first_component = normalized.split('/', 1)[0].casefold()
    return first_component in IGNORABLE_RECOVERY_DIRECTORIES
...
def _validate_content_types(archive, names, issues, parsed):
    ...
    for part in sorted(names - {'[Content_Types].xml'}):
        if is_ignorable_recovery_part(part):
            continue
...
def _validate_comments(archive, names, relationships_by_owner, issues, parsed):
    comment_parts = sorted((name for name in names if name.startswith('ppt/comments/') and name.endswith('.xml')))
    if not comment_parts:
        return
    authors_part = 'ppt/commentAuthors.xml'
    if authors_part not in names:
        issues.append('classic comments exist but ppt/commentAuthors.xml is missing')
        return
    author_root = _parse_xml(archive, authors_part, issues, parsed)
    if author_root is None:
        return
    author_ids = [author.get('id') for author in author_root.findall(qname(NS_PRESENTATION, 'cmAuthor'))]
    missing_author_ids = [value for value in author_ids if value is None]
    if missing_author_ids:
        issues.append('ppt/commentAuthors.xml: every comment author requires an id')
    duplicate_author_ids = sorted((value for value, count in Counter((value for value in author_ids if value is not None)).items() if count > 1))
    if duplicate_author_ids:
        issues.append(f'ppt/commentAuthors.xml: duplicate comment author IDs {duplicate_author_ids}')
    known_author_ids = {value for value in author_ids if value is not None}
    presentation_relationships = relationships_by_owner.get('ppt/presentation.xml', {})
    author_relationships = [relationship for relationship in presentation_relationships.values() if relationship['type'] == RELATIONSHIP_TYPES['commentAuthors'] and relationship['resolved'] == authors_part]
    if len(author_relationships) != 1:
        issues.append('ppt/presentation.xml: classic comments require exactly one commentAuthors relationship')
    seen_comment_ids = set()
    for part in comment_parts:
        comment_root = _parse_xml(archive, part, issues, parsed)
        if comment_root is None:
            continue
        for comment in comment_root.findall(qname(NS_PRESENTATION, 'cm')):
            author_id = comment.get('authorId')
            index = comment.get('idx')
            if author_id not in known_author_ids:
                issues.append(f'{part}: comment references unknown authorId {author_id!r}')
            if index is None:
                issues.append(f'{part}: comment is missing idx')
                continue
            identity = (author_id, index)
            if identity in seen_comment_ids:
                issues.append(f'{part}: duplicate classic comment identity authorId={author_id!r}, idx={index!r}')
            seen_comment_ids.add(identity)
...
def validate_pptx_source(source):
    ...
    try:
        with ZipFile(source) as archive:
            ...
            for owner, relationships in relationships_by_owner.items():
                for relationship_id, relationship in relationships.items():
                    resolved = relationship['resolved']
                    if resolved and is_ignorable_recovery_part(resolved):
                        issues.append(f'{_relationship_part_name(owner)}: {relationship_id} references recovery artifact {resolved!r}, which cannot be discarded safely')
            ...
            if REQUIRED_PARTS <= names:
                ...
                _validate_comments(archive, names, relationships_by_owner, issues, parsed)
...
class MergeWorkspace:
    ...
    def _remove_ignorable_parts(self):
        """Drop known unreferenced recovery artifacts from the output package."""
        for candidate in self.root.iterdir():
            if not is_ignorable_recovery_part(candidate.name + '/'):
                continue
            if candidate.is_dir():
    ...
    def _create_directories(self):
        ...
        self.comments_dir = self.ppt_dir / 'comments'
        self.comments_rels_dir = self.comments_dir / '_rels'
        self.theme_dir = self.ppt_dir / 'theme'
        self.diagrams_dir = self.ppt_dir / 'diagrams'
        for d in [self.slides_dir, self.slides_rels_dir, self.layouts_dir, self.layouts_rels_dir, self.masters_dir, self.masters_rels_dir, self.charts_dir, self.charts_rels_dir, self.drawings_dir, self.media_dir, self.embeddings_dir, self.tags_dir, self.notes_dir, self.notes_rels_dir, self.comments_dir, self.comments_rels_dir, self.theme_dir, self.diagrams_dir]:
...
class SlideAssetImporter:
    ...
    def __init__(self, package):
        ...
        self.comment_counter = len(list(self.package.comments_dir.glob('comment*.xml')))
    ...
    def import_relationships(self, src_slide, source_root, source_identity, slide_link_map, new_slide_name, slide_index):
        ...
        for rel in tree.getroot().findall(qname(NS_PACKAGE_RELATIONSHIPS, 'Relationship')):
            ...
            if 'slideLayout' in kind:
                ...
                self.import_comments(source_root, src_slide, target, rel)
    ...
    @staticmethod
    def comment_author_identity(author):
        presence = next((element for element in author.iter() if element.tag.rsplit('}', 1)[-1] == 'presenceInfo'), None)
        return (author.get('name', ''), author.get('initials', ''), presence.get('userId', '') if presence is not None else '', presence.get('providerId', '') if presence is not None else '')

    def prepare_comment_authors(self, source_root):
        source_path = source_root / 'ppt' / 'commentAuthors.xml'
        if not source_path.exists():
            return (None, {}, {})
        source_tree = ET.parse(source_path)
        source_root_element = source_tree.getroot()
        destination_path = self.package.ppt_dir / 'commentAuthors.xml'
        if destination_path.exists():
            destination_tree = ET.parse(destination_path)
        else:
            destination_tree = ET.ElementTree(ET.Element(source_root_element.tag, dict(source_root_element.attrib)))
        destination_root = destination_tree.getroot()
        destination_authors = list(destination_root.findall(qname(NS_PRESENTATION, 'cmAuthor')))
        identities = {self.comment_author_identity(author): author for author in destination_authors}
        used_ids = {int(author.get('id')) for author in destination_authors if (author.get('id') or '').isdigit()}
        source_to_destination = {}
        destination_by_id = {author.get('id'): author for author in destination_authors if author.get('id')}
        for source_author in source_root_element.findall(qname(NS_PRESENTATION, 'cmAuthor')):
            identity = self.comment_author_identity(source_author)
            destination_author = identities.get(identity)
            if destination_author is None:
                author_id = 0
                while author_id in used_ids:
                    author_id += 1
                used_ids.add(author_id)
                destination_author = copy.deepcopy(source_author)
                destination_author.set('id', str(author_id))
                destination_author.set('lastIdx', '0')
                destination_author.set('clrIdx', str(author_id))
                destination_root.append(destination_author)
                identities[identity] = destination_author
                destination_by_id[str(author_id)] = destination_author
            source_to_destination[source_author.get('id')] = destination_author.get('id')
        relationship_type = NS_DOCUMENT_RELATIONSHIPS + '/commentAuthors'
        has_relationship = any((relationship.get('Type') == relationship_type for relationship in self.package.relationships_root.findall(qname(NS_PACKAGE_RELATIONSHIPS, 'Relationship'))))
        if not has_relationship:
            relationship = ET.SubElement(self.package.relationships_root, qname(NS_PACKAGE_RELATIONSHIPS, 'Relationship'))
            relationship.set('Id', f'rId{self.package.next_relationship_id}')
            self.package.next_relationship_id += 1
            relationship.set('Type', relationship_type)
            relationship.set('Target', 'commentAuthors.xml')
        self.register_source_content_type(source_root, source_path, destination_path)
        return (destination_tree, source_to_destination, destination_by_id)

    def import_comments(self, source_root, owner_part, target, rel):
        source_comment = self.resolve_source_part(source_root, owner_part, target)
        if not source_comment.exists():
            return
        authors_tree, author_map, destination_authors = self.prepare_comment_authors(source_root)
        self.comment_counter += 1
        comment_name = f'comment{self.comment_counter}.xml'
        destination_comment = self.package.comments_dir / comment_name
        comment_tree = ET.parse(source_comment)
        for comment in comment_tree.getroot().findall(qname(NS_PRESENTATION, 'cm')):
            destination_author_id = author_map.get(comment.get('authorId'))
            if destination_author_id is None:
                continue
            comment.set('authorId', destination_author_id)
            author = destination_authors[destination_author_id]
            try:
                next_index = int(author.get('lastIdx', '0')) + 1
            except ValueError:
                next_index = 1
            comment.set('idx', str(next_index))
            author.set('lastIdx', str(next_index))
        comment_tree.write(destination_comment, xml_declaration=True, encoding='utf-8')
        self.register_source_content_type(source_root, source_comment, destination_comment)
        if authors_tree is not None:
            authors_tree.write(self.package.ppt_dir / 'commentAuthors.xml', xml_declaration=True, encoding='utf-8')
        source_relationships = self.relationship_path(source_comment)
        if source_relationships.exists():
            relationships = ET.parse(source_relationships)
            self.rewrite_generic_relationships(relationships, source_root, source_comment, destination_comment)
            save_rels_xml(relationships, self.package.comments_rels_dir / f'{comment_name}.rels')
        rel.set('Target', f'../comments/{comment_name}')
...
def write_package(package_root: Path, output: Path, validator=None) -> None:
    """Write and validate beside the output, then replace it atomically."""
    output = Path(output)
    staged = None
    try:
...
def merge_pptx_files_autonomous(input_pptx_files, output_pptx_path):
    ...
    with tempfile.TemporaryDirectory(prefix='pptx_merge_') as temporary:
        ...
        write_package(package.root, output, validator=validate_pptx_source)
