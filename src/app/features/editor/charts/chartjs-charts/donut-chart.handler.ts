...
export class ChartJsDonutChartHandler implements ChartJsChartTypeHandler {
  ...
  convertToChartJsConfig(
    ...
    // For donut charts, we use the first series and combine it with labels
    const firstSeries = series[0] || { name: '', data: [] };
    const chartLabels = labels || firstSeries.data.map((_, i) => `Item ${i + 1}`);
    const defaultColors = this.getDefaultColors();
    
    // Combine labels and data for donut chart format
    const datasets = [{
    ...
    } as any]; // Type assertion for cutout property
    ...
    return {
      ...
      } as any, // Type assertion to allow cutout in options
    };
  }
