import { ChartConfiguration } from 'chart.js';
import { ChartSeries } from '../../../../models/chart-data.model';
import { ChartJsChartTypeHandler } from './chartjs-chart-type.interface';

export class ChartJsPieChartHandler implements ChartJsChartTypeHandler {
  readonly chartType = 'pie';
  readonly chartJsType: ChartConfiguration['type'] = 'pie';

  convertToChartJsConfig(
    series: ChartSeries[],
    labels: string[] | undefined,
    title?: string,
    xAxisLabel?: string,
    yAxisLabel?: string,
    showLegend?: boolean,
    legendPosition?: 'top' | 'bottom' | 'left' | 'right',
    colors?: string[]
  ): ChartConfiguration {
    // For pie charts, we use the first series and combine it with labels
    const firstSeries = series[0] || { name: '', data: [] };
    const chartLabels = labels || firstSeries.data.map((_, i) => `Item ${i + 1}`);
    const defaultColors = this.getDefaultColors();
    
    // Combine labels and data for pie chart format
    const datasets = [{
      label: firstSeries.name,
      data: firstSeries.data,
      backgroundColor: firstSeries.data.map((_, index) => 
        colors?.[index] || defaultColors[index % defaultColors.length]
      ),
      borderColor: '#fff',
      borderWidth: 2,
    }];

    return {
      type: 'pie',
      data: {
        labels: chartLabels,
        datasets,
      },
      options: {
        responsive: true,
        maintainAspectRatio: false,
        plugins: {
          title: {
            display: !!title,
            text: title || '',
          },
          legend: {
            display: showLegend !== false,
            position: legendPosition || 'top',
          },
        },
      },
    };
  }

  private getDefaultColors(): string[] {
    return [
      'rgba(124, 181, 236, 0.8)',
      'rgba(67, 67, 72, 0.8)',
      'rgba(144, 237, 125, 0.8)',
      'rgba(247, 163, 92, 0.8)',
      'rgba(128, 133, 233, 0.8)',
      'rgba(241, 92, 128, 0.8)',
      'rgba(228, 211, 84, 0.8)',
      'rgba(43, 144, 143, 0.8)',
      'rgba(244, 91, 91, 0.8)',
      'rgba(145, 232, 225, 0.8)',
    ];
  }
}
